...
- **Размещение**: один пользователь может публиковать несколько объявлений; каждое создание добавляет новую строку без перезаписи предыдущих записей пользователя.
...
- **Размещение**: пользователь может держать несколько объявлений по жилью одновременно; новые записи не должны перезаписывать существующие по тому же `user_id`.
...
- **Размещение**: для вакансий и резюме один пользователь может создавать несколько карточек, каждая сохраняется отдельной строкой и не ограничена лимитом «одна запись на пользователя».
...
- `GET /api/profile` — базовый профиль пользователя (`gender`, `birthDate`, `city`, служебные поля `telegramUsername`/`displayName`) и статистика по анкетам/объявлениям.
- `PUT /api/profile` — частичное обновление базового профиля с валидацией возраста (14–120 лет); отдельного флага «Профиль 18+» нет.
- `GET /api/dating/profile` — вернуть анкету текущего пользователя или `null` (собирает данные анкеты и базового профиля). Поле `comment` используется как текст «О себе».
- `PUT /api/dating/profile` — создать/обновить анкету. Валидация: непустые `nickname`, `looking_for`, `offer`, минимум один `purposes`; при включённой анкете (`is_active = true`) требуется заполненный базовый профиль. Добавлены предпочтения `preferred_genders`, возрастной диапазон и режим города, а также флаги показа объявлений по разделам. Поле `comment` хранит самопрезентацию анкеты.
- `GET /api/dating/feed` — лента активных анкет без уже просмотренных/свайпнутых. Параметры: `limit` (<=50), `purposes` (фильтр по пересечению целей, можно перечислять через запятую). Выбирает только анкеты с `is_active=true`, актуальные за 90 дней, подходящие по полу, возрасту, городу (для `same_city`) и возрастным ограничениям по дате рождения: пользователи младше 18 видят только анкеты <18 лет, пользователи 18+ видят только анкеты 18+.
...
- **Изменения**: добавлена единая модель профиля пользователя (`gender`, `birth_date`, `city`) с API `/api/profile`; анкета знакомств расширена предпочтениями по полу, возрасту и гео, флагами показа объявлений по разделам; лента `/api/dating/feed` фильтрует анкеты по полу, возрасту, городу и возрастным правилам 18+/minor; Mini App получила страницу `/profile` с вкладками «Основное», «Знакомства», «Мои объявления» и кнопки перехода из раздела знакомств.
...
### Упрощение профиля: перенос «О себе» и отказ от флага 18+ (дата: 2026-07-13)
- **Файлы**: `docs/db/016_remove_about_and_is_adult_profile.sql`, `apps/miniapp/app/api/profile/route.ts`, `apps/miniapp/app/api/dating/profile/route.ts`, `apps/miniapp/app/api/dating/feed/route.ts`, `apps/miniapp/app/profile/page.tsx`, `apps/miniapp/app/dating/page.tsx`, `README.md`.
- **Изменения**: поле `about` и флаг `is_adult_profile` удалены из `users`; миграция переносит старый `about` в `dating_profiles.comment` (если пусто) и удаляет колонки. Базовый профиль хранит только пол, дату рождения, город и служебные поля Telegram. Самопрезентация перенесена в анкету знакомств (`comment`), UI вкладки «Основное» показывает только базовые поля, вкладка «Знакомства» редактирует «Комментарий / О себе». Лента знакомств разделяет взрослых и несовершеннолетних только по дате рождения.
  - **Поток данных**: пользователь заполняет пол/дату рождения/город на `/profile` → `PUT /api/profile` сохраняет эти поля в `users` → вкладка «Знакомства» обновляет `comment` и остальные параметры через `PUT /api/dating/profile` → лента `/api/dating/feed` фильтрует кандидатов по полу, возрасту, гео и разделяет <18 и 18+ только по дате рождения.
...
### Исправление множественных объявлений Маркет/Жильё/Работа (дата: 2026-06-01)
- Затронутые файлы и папки:
  - `docs/db/012_fix_unique_constraints_listings.sql`
  - `README.md`
- Краткое описание:
  - Добавлена миграция, удаляющая любые уникальные индексы по `user_id` на таблицах объявлений, чтобы один пользователь мог хранить несколько карточек в каждом разделе.
  - README дополнен уточнением, что создание новых объявлений добавляет новые строки без перезаписи предыдущих записей пользователя.
  - Поток данных: пользователь создаёт объявление → API `/api/(market|housing|jobs)/listings` вставляет новую строку без ограничений по `user_id` → ленты `/api/.../listings` возвращают все активные объявления для отображения в Mini App.
